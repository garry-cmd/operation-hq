...
export default function SpaceSwitcher({ spaces, activeSpaceId, objectives, roadmapItems, onSelect, onSpaceCreated, onSpaceUpdated, onSpaceDeleted }: Props) {
  ...
  function openDropdown() {
    ...
    setOpen(o => !o); setAdding(false); setEditingId(null)
  }
  ...
  async function saveNew() {
    ...
    try {
      ...
      onSelect(created.id)
      setOpen(false)
    } catch (err) { console.error('saveNew space failed:', err) }
  ...
  async function saveEdit() {
    ...
    } catch (err) { console.error('saveEdit space failed:', err) }
  ...
  async function deleteSpace(spaceId: string) {
    ...
    } catch (err) { console.error('deleteSpace failed:', err) }
    setDeleting(false)
  }
    ...
    <div style={{ padding: '10px 12px', borderBottom: '1px solid var(--navy-600)' }}>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        <button onClick={() => setEditingId(null)}
          style={{ flex: 1, padding: '6px', background: 'var(--navy-600)', border: 'none', borderRadius: 7, color: 'var(--navy-300)', fontSize: 12, cursor: 'pointer' }}>Cancel</button>
  ...
  const dropdownStyle: React.CSSProperties = isMobile && dropdownPos
    ? { position: 'fixed', top: dropdownPos.top, left: dropdownPos.left, minWidth: 230, background: 'var(--navy-700)', border: '1px solid var(--navy-500)', borderRadius: 14, overflow: 'hidden', zIndex: 200, boxShadow: '0 8px 32px rgba(0,0,0,.6)' }
    : { position: 'absolute', top: 'calc(100% + 6px)', left: 0, minWidth: 230, background: 'var(--navy-700)', border: '1px solid var(--navy-500)', borderRadius: 14, overflow: 'hidden', zIndex: 60, boxShadow: '0 8px 32px rgba(0,0,0,.4)' }
                ...
                {isEditing ? editForm(space.id) : (
                  <button onClick={() => { onSelect(space.id); setOpen(false) }}
                    ...
                    <button onClick={e => openEdit(space, e)}
                      style={{ width: 24, height: 24, borderRadius: 6, background: 'var(--navy-500)', border: 'none', color: 'var(--navy-300)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, opacity: isActive ? 0.7 : 0.55 }}
