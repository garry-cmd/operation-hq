...
export default function Focus({ objectives, roadmapItems, krs, actions, setActions, weekStart, setWeekStart, toast }: Props) {
  ...
  const allMilestones = krs.filter(k => activeItems.some(i => i.id === k.roadmap_item_id))
  ...
  // Milestones with no actions this week — the fallback nudge
  const unplanned = allMilestones.filter(m => !weekActions.some(a => a.quarterly_kr_id === m.id))
  ...
  // Enrich actions with objective context
  ...
  return (
    <>
      ...
      <div>
        ...
        {/* ── Empty state: Plan your week prompt ── */}
        {taskTotal === 0 && activeItems.length > 0 && (
          <div style={{ background: 'var(--navy-800)', border: '1px solid var(--navy-600)', borderRadius: 16, padding: '24px 20px', textAlign: 'center', marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--navy-400)', lineHeight: 1.6, marginBottom: 20 }}>
              Walk through each milestone and decide what you're doing about it this week. Takes 2 minutes.
            </div>
            <button onClick={() => setPlanning(true)} className="btn-primary" style={{ width: '100%', fontSize: 14, marginBottom: 10 }}>
              Start planning →
            </button>
            <p style={{ fontSize: 12, color: 'var(--navy-500)' }}>
              Or use the <strong style={{ color: 'var(--accent)' }}>+</strong> button to add actions manually
            </p>
          </div>
        )}
        ...
        {taskTotal === 0 && activeItems.length === 0 && (
          <div style={{ textAlign: 'center', padding: '48px 0', color: 'var(--navy-400)', fontSize: 14, lineHeight: 1.6 }}>
            ...
            Add objectives and key results first, then plan your week here.
          </div>
        )}
        ...
        {enriched.map(({ action, obj }) => (
          <div key={action.id}
            style={{
              ...
              opacity: action.completed ? .55 : 1, transition: 'opacity .15s',
            }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 15, fontWeight: 500, color: action.completed ? 'var(--navy-400)' : 'var(--navy-50)',
                textDecoration: action.completed ? 'line-through' : 'none', lineHeight: 1.35, marginBottom: 3,
        ...
        {/* ── Fallback: unplanned milestones nudge ── */}
          ...
          <div style={{ marginTop: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--navy-400)', textTransform: 'uppercase', letterSpacing: '.5px' }}>
                {unplanned.length} milestone{unplanned.length > 1 ? 's' : ''} with nothing planned
              </span>
            </div>
            {unplanned.map(ms => {
              const item = activeItems.find(i => i.id === ms.roadmap_item_id)
              const obj = objectives.find(o => o.id === item?.annual_objective_id)
              return (
                <UnplannedRow key={ms.id} milestone={ms} objective={obj}
                  onAdd={async (title) => {
                    const { data } = await supabase.from('weekly_actions')
                      .insert({ quarterly_kr_id: ms.id, title, week_start: weekStart }).select().single()
...
/* Inline unplanned row with quick-add */
function UnplannedRow({ milestone, objective, onAdd }: {
  milestone: QuarterlyKR
  ...
  return (
    <div style={{ background: 'var(--navy-800)', border: '1px solid var(--navy-600)', borderRadius: 12, marginBottom: 7, overflow: 'hidden' }}>
      <div style={{ padding: '11px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          {objective && (
            <div style={{ fontSize: 11, color: 'var(--navy-400)', marginBottom: 2 }}>{objective.name}</div>
          )}
          <div style={{ fontSize: 13, color: 'var(--navy-200)', lineHeight: 1.35 }}>{milestone.title}</div>
        </div>
        <button onClick={() => setOpen(o => !o)}
          style={{ fontSize: 12, fontWeight: 600, color: 'var(--accent)', background: 'var(--accent-dim)', border: 'none', borderRadius: 8, padding: '6px 12px', cursor: 'pointer', flexShrink: 0, whiteSpace: 'nowrap' }}>
      ...
      {open && (
        <form onSubmit={save} style={{ padding: '0 14px 12px', borderTop: '1px solid var(--navy-600)' }}>
          <input ref={ref} value={value} onChange={e => setValue(e.target.value)}
            placeholder="What are you doing about this?"
